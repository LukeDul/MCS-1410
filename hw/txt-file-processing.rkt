...
; returns the shortest list of a list of lists
(define (shortest-line-in-a-file filename)
  (local [(define 2d-list (read-words/line filename))]
    (foldl (lambda (line acc)
             (cond [(empty? line) acc]
                   [(empty? acc) line]
                   [(< (length line) (length acc)) line]
                   [else acc]))
           empty
           2d-list))) 
 ...
 (check-expect (shortest-line-in-a-file  "txt-files/txt-file-processing/problem3/test1.txt") 
      (list "Such" "indomitable" "effervescence" "accomplishes" "magnificance"))
 (check-expect (shortest-line-in-a-file "txt-files/txt-file-processing/problem3/test2.txt") 
       (list "Hourglass") )
